...
short-description: Reverse-engineer and clone one or more websites as pixel-perfect replicas
argument-hint: "<url1> [<url2> ...]"
no-args-hint: "the target URL or URLs provided by the user"
user-invocable: true
---
...
- **Customization:** None — pure emulation. Making the result editable is `/templatize`'s job,
  and rebranding it is `/customize-site`'s. Do not start either one early.
...
## Phase 6: Hand Off to /templatize

A clone whose copy is hard-coded in JSX is only half the deliverable — every later text change
becomes a code change. Once visual QA passes, run `/templatize <page-key>` for each page you
built.

That skill lifts every string, image, link, and repeated block out of the components into a zod
schema plus `content/<page-key>.json`, registers the page, and generates a placeholder variant.
Afterwards the page renders identically but is editable at `/edit`, and `/customize-site` can
rebrand it without touching a component.

Do not start templatizing before visual QA passes. Templatize is verified by comparing
before/after screenshots, and that check is worthless if the "before" was already wrong.
...
- Whether `/templatize` has been run for each page, and if not, why
