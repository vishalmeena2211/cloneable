---
name: templatize
description: Convert built page components into a content-driven template — lift every string, image, and link into a typed zod schema plus a JSON content file, then rewire the components to read from it. Use this after /clone-website, or whenever hard-coded copy needs to become editable. Also triggers on "make this editable", "extract the content", "turn this into a template".
short-description: Lift hard-coded copy out of components into a typed, editable content layer
argument-hint: "[page-key ...]"
no-args-hint: "the page keys provided by the user, or every page that still has hard-coded copy"
user-invocable: true
---
